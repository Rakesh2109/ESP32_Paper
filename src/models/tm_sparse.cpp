...
/* ============================ LUTs =============================== */
static void tm_build_luts_(TMSparse* tm){
...
/* ======================== NZ list helper ========================= */
#if TM_USE_NZ
static inline void update_nz_list_(TMSparse* tm, TMClause* c, int w) {
  (void)tm;
...
/* ======================== ID index helper ========================= */
static bool ensure_id_index_(TMSparse* tm, uint32_t id) {
...
/* ===================== Pack X → 32-bit words ===================== */
static TM_HOT void tm_pack_X_to_words_(TMSparse* tm,
#if TM_INPUT_IS_MSB_PACKED
...
static inline int tm_clip_class_sum_(const TMSparse* tm, int s) {
  if (s > tm->threshold) return tm->threshold;
  if (s < -tm->threshold) return -tm->threshold;
  return s;
}

static TM_HOT int tm_calculate_clause_output_update_(TMSparse* tm, const uint8_t* X) {
...
static TM_HOT int tm_calculate_clause_output_predict_(TMSparse* tm, const uint8_t* X) {
  tm_pack_X_to_words_(tm, X);
  const int W = tm->words_per_feat;
  const uint32_t* __restrict__ xw = tm->xwords;
  uint8_t* __restrict__ out = tm->clause_output;
...
static inline void tm_rand2_u24_(TMSparse* tm, uint32_t* r0, uint32_t* r1) {
  const uint64_t r = tm_rand_u64_(tm->rng_state);
  *r0 = (uint32_t)(r >> 40);
  *r1 = (uint32_t)((r >> 16) & 0xFFFFFFu);
}
static inline bool bern_1_over_s_(TMSparse* tm, tm_s_thresh_t thr_1_div_s) {
  return tm_rand_u24_(tm->rng_state) <= thr_1_div_s;
}
static inline bool bern_s1_over_s_(TMSparse* tm, tm_s_thresh_t thr_1_div_s) {
  return tm_rand_u24_(tm->rng_state) > thr_1_div_s;
}
#else
...
static TM_HOT void tm_type_i_feedback_(TMSparse* tm, const uint32_t* Xw, TMClause* c,
...
static TM_HOT void tm_type_ii_feedback_(TMSparse* tm, const uint32_t* Xw, TMClause* c) {
  const int F = tm->num_features, N = tm->num_states, W = tm->words_per_feat;
  uint8_t* __restrict__ pos_state = c->pos_state;
  uint8_t* __restrict__ neg_state = c->neg_state;
  uint32_t* __restrict__ pos_mask = c->pos_mask;
...
static bool ensure_active_scratch_(TMSparse* tm, int need) {
...
static bool realloc_capacity_(TMSparse* tm, int new_cap) {
  const int F = tm->num_features, W = tm->words_per_feat;
...
static void maybe_shrink_(TMSparse* tm) {
...
/* ================= Clause lookup ========================= */
static int find_clause_idx_(TMSparse* tm, uint32_t id) {
...
/* ====================== Clause management ========================= */
static void init_clause_random_(TMSparse* tm, TMClause* c) {
...
static void remove_from_active_(TMSparse* tm, TMClause* c) {
...
bool tm_clause_remove(TMSparse* tm, uint32_t clause_id) {
...
bool tm_clause_freeze(TMSparse* tm, uint32_t clause_id) {
  int idx = find_clause_idx_(tm, clause_id);
  if (idx < 0) return false;
  TMClause* c = &tm->clauses[idx];
  c->frozen = 1;
...
bool tm_clause_thaw(TMSparse* tm, uint32_t clause_id) {
  int idx = find_clause_idx_(tm, clause_id);
  if (idx < 0) return false;
...
bool tm_clause_activate(TMSparse* tm, uint32_t clause_id) {
  int idx = find_clause_idx_(tm, clause_id);
  if (idx < 0) return false;
  TMClause* c = &tm->clauses[idx];
...
bool tm_clause_deactivate(TMSparse* tm, uint32_t clause_id) {
  int idx = find_clause_idx_(tm, clause_id);
  if (idx < 0) return false;
...
static int tm_calculate_clause_output_from_words_update_(TMSparse* tm, const uint32_t* Xw);
static int tm_calculate_clause_output_from_words_predict_(TMSparse* tm, const uint32_t* Xw);
...
static TM_HOT int tm_calculate_clause_output_from_words_update_(TMSparse* tm, const uint32_t* Xw) {
  const int W = tm->words_per_feat;
  uint8_t* __restrict__ out = tm->clause_output;
  TMClause* const* __restrict__ act = (TMClause* const*)tm->active;
  int s = 0;
...
static TM_HOT int tm_calculate_clause_output_from_words_predict_(TMSparse* tm, const uint32_t* Xw) {
  const int W = tm->words_per_feat;
  uint8_t* __restrict__ out = tm->clause_output;
  TMClause* const* __restrict__ act = (TMClause* const*)tm->active;
  int s = 0;
...
/* ========================== Utilities ============================ */
uint32_t tm_sparse_memory_usage_active(const TMSparse* tm) {
  if (!tm) return 0;
  const int F = tm->num_features, W = tm->words_per_feat;
...
bool tm_init(TsetlinMachine* tm, int features, int clauses, int threshold, uint32_t seed) {
  if (!tm) return false;
  if (!tm_sparse_init(&tm->core, features, threshold, seed)) return false;
